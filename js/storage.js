...
function getDueKanji(kanjiList) {
  ...
  // SRS: solo kanji già studiati e in scadenza (o difficili), MAI i nuovi
  // I kanji difficili entrano sempre indipendentemente dalla dueDate
  return withSRS
    .filter(k => k.srs.totalSeen > 0 && (k.srs.isDifficult || new Date(k.srs.dueDate) <= now))
    .sort((a, b) => {
      // difficili sempre in testa
      if (a.srs.isDifficult && !b.srs.isDifficult) return -1;
      if (!a.srs.isDifficult && b.srs.isDifficult) return  1;
      // poi per data di scadenza (più vecchi prima)
      const dd = new Date(a.srs.dueDate) - new Date(b.srs.dueDate);
      if (dd !== 0) return dd;
      // a parità di data, prima quelli con accuratezza più bassa
      const aE = 1 - a.srs.totalCorrect / a.srs.totalSeen;
      const bE = 1 - b.srs.totalCorrect / b.srs.totalSeen;
      return bE - aE;
    });
}
...
function get10kQueue(wordList) {
  ...
  // SRS puro: solo parole già viste e in scadenza
  return withSRS
    .filter(w => w.srs.totalSeen > 0 && new Date(w.srs.dueDate) <= now)
    .sort((a,b) => new Date(a.srs.dueDate) - new Date(b.srs.dueDate));
}
